...
// Cari lisensi AKTIF (belum expired) yang terkunci ke sebuah grup (jid).
// Dipakai untuk mendeteksi "grup sudah punya bot / sudah terdaftar".
export async function getLicenseByGroup(groupJid) {
...
// Group lock: kunci lisensi ke sebuah grup (jid). Kosong = lepas kunci.
export async function setGroupLock(key, groupJid, groupName) {
    let found = null
    await update("licenses", (list) =>
        list.map((l) => {
            if (l.key === key) {
                l.groupJid = groupJid ? String(groupJid).trim() : null
                if (groupName !== undefined) l.groupName = groupName || null
                found = l
            }
            return l
        })
...
// Set maksimal anggota grup (khusus private). null = tanpa batas.
export async function setMaxMembers(key, max) {
    let found = null
    await update("licenses", (list) =>
        list.map((l) => {
            if (l.key === key) {
                const n = Number(max)
                l.maxMembers = n > 0 ? Math.floor(n) : null
                found = l
            }
            return l
        })
...
export async function heartbeat(key, meta = {}) {
    ...
    // Sertakan cap waktu perubahan config → bot bisa auto-reload bila berubah.
    let configUpdatedAt = 0
    try {
        const cfgs = await read("configs")
        const entry = cfgs.find((c) => c.key === key)
        configUpdatedAt = entry?.updatedAt || 0
    } catch {}
    return { valid: true, status: "active", expiresAt: lic.expiresAt, configUpdatedAt }
}
